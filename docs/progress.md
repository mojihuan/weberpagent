...
- **结果**: 3 轮测试全部失败
- **任务清单**:
  - [x] 6.1 分析 Phase 4 采购单场景失败原因 ✅
  - [x] 6.2 Prompt 优化（侧边栏导航、表单填写规则） ✅
  - [x] 6.3 运行采购单场景测试 ✅
  - [x] 6.4 验证测试并生成报告 ✅
...
| 轮次 | 步数 | 耗时 | 结果 |
|------|------|------|------|
| 第 1 轮 | 25 | 8.3 分钟 | ❌ 超过最大步数 |
| 第 2 轮 | 25 | 83 分钟 | ❌ 超过最大步数 |
| 第 3 轮 | 0 | - | ❌ API 错误 |

**失败原因分析**:
1. **侧边栏导航循环**: Agent 登录成功后，在侧边栏"商品采购"菜单上陷入循环
2. **子菜单未展开**: 点击菜单后子菜单没有出现，Agent 无法识别子菜单元素
3. **循环恢复机制无效**: 等待页面加载策略无法解决菜单展开问题

**后续优化建议**:
1. 增强侧边栏导航的等待策略（等待子菜单元素出现）
2. 考虑使用悬停（hover）代替点击展开菜单
3. 增加菜单展开状态检测（检查子元素是否可见）
...
- **策略**: 分模块突破（M1→M2→M3→M4→M5），每个模块独立验证后整合
- **任务清单**:
  - [x] 7.1 创建测试目录结构和运行脚本 ✅
  - [x] 7.2 实现 hover 动作支持 ✅
  - [x] 7.3 编写 M1 登录验证测试 ✅
  - [x] 7.4 编写 M2 侧边栏一级菜单测试 ✅
  - [x] 7.5 编写 M3 侧边栏二级菜单测试 ✅
  - [x] 7.6 编写 M4 表单填写测试 ✅
  - [x] 7.7 编写 M5 提交验证测试 ✅
  - [x] 7.8 编写整合测试 ✅
  - [ ] 7.9 运行测试验证（待执行）
- **核心改进**:
  - 新增 `hover` 动作类型（用于菜单悬停展开）
  - 分模块测试架构（独立验证、精准定位问题）
