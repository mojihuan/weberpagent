...
# ---------------------------------------------------------------------------
# FORM-06: Logistics fee column selection verification
# ---------------------------------------------------------------------------


@pytest.mark.skipif(not _has_api_key(), reason="DASHSCOPE_API_KEY not configured")
@pytest.mark.asyncio
async def test_e2e_column_selection_logistics_fee(api_client):
    """FORM-06: Verify Agent selects logistics fee column.

    Creates a task targeting the sales outbound page, asking the Agent to find
    a specific row by IMEI and fill the logistics fee with 20. Verifies that:
    - Agent reasoning shows awareness of "物流费用" column
    - If run succeeds, column selection is validated
    - If run fails, failure was NOT due to wrong column without logistics fee awareness
    - Either an input action with value 20 exists, or the run succeeded
    """
    # Step 1: Create task
    task_payload = {
        "name": "E2E列选择验证-物流费用",
    ...
    # 5a: At least one step should mention "物流费用" in reasoning
    has_logistics_fee_awareness = any("物流费用" in r for r in reasoning_texts)

    # 5b: An input action with value "20" should exist
    has_value_20_input = any("20" in a and "input" in a.lower() for a in actions)

    # 5c: Column selection correctness check.
    run_succeeded = final_run["status"] == "success"

    # PRIMARY: Agent shows awareness of logistics fee column
    assert has_logistics_fee_awareness, (
        "No step reasoning mentions '物流费用' -- "
        "Agent may not be using column annotations. "
        f"All reasoning: {reasoning_texts}"
    )

    # CRITICAL: If run succeeded, column selection is validated.
    # If run failed, check that failure was NOT due to wrong column targeting.
    if not run_succeeded:
        for i, action in enumerate(actions):
            if "input" in action.lower() or "click" in action.lower():
                reasoning = reasoning_texts[i] if i < len(reasoning_texts) else ""
                if "物流费用" not in reasoning and any(
                    col in reasoning for col in ("销售金额", "利润", "成本")
                ):
                    pytest.fail(
                        f"Run failed and step {i} targets a different column without "
                        f"logistics fee awareness: action={action}, reasoning={reasoning}"
                    )

    # SECONDARY: Value 20 was input (strong signal but Agent may use evaluate JS)
    if not has_value_20_input:
        if final_run["status"] != "success":
            pytest.fail(
                "Run failed and no input action with value 20 found -- "
                "Agent likely did not fill the logistics fee correctly"
            )
