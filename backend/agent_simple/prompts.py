...
| click | 元素文本 | null | 点击目标元素 |
| input | 元素文本 | 输入内容 | 在目标元素中输入文本 |
...
```json
{"thought": "你的思考过程", "action": "动作类型", "target": "目标元素描述或null", "value": "URL或输入值或null", "done": false}
```

## 重要规则

1. navigate 动作的 URL 必须放在 value 字段，target 设为 null
2. click 动作的元素文本放在 target 字段
3. input 动作的元素文本放在 target，输入内容放在 value
4. 任务完成后设置 done: true
