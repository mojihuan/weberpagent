...
**不要使用 `networkidle`！** 很多页面有 WebSocket 或长轮询，导致它永远不会触发。

```python
# ❌ 错误：可能导致超时
await page.wait_for_load_state('networkidle')

# ✅ 正确：使用 domcontentloaded 或直接跳过等待
await page.wait_for_load_state('domcontentloaded')
# 或直接使用固定等待
await page.wait_for_timeout(500)
# 或等待特定元素出现
await page.wait_for_selector('.form-container')
```
...
#### 方式1：点击打开下拉 + 使用相对位置选择（推荐！不知道选项名称时使用）
```python
# 步骤1：点击打开下拉框
await page.locator('[placeholder="请选择供应商"]').click()
# 步骤2：等待下拉面板可见（重要！确保下拉框已打开）
await page.wait_for_selector('.el-select-dropdown:visible', timeout=5000)
# 步骤3：使用 :visible 选择器选择可见的选项（重要！）
await page.locator('.el-select-dropdown__item:visible').first.click()
# 或 Ant Design
await page.locator('.ant-select-item:visible').first.click()
# 或选择第二个可见选项
await page.locator('.el-select-dropdown__item:visible').nth(1).click()
```

#### ⚠️ 重要：必须使用 :visible 选择器！
Element UI 下拉选项即使不可见也能被定位到（来自已关闭的下拉框），所以必须加 `:visible` 确保选项是可见的。

#### 方式2：已知选项名称时，点击打开下拉 + 点击具体选项
```python
# 仅当你从元素列表中看到确切的选项文本时才使用此方法
await page.locator('[placeholder="请选择供应商"]').click()
await page.wait_for_timeout(500)
await page.get_by_text('已知选项名称', exact=True).click()
```

#### 方式3：使用 select_option（适用于原生 <select> 元素）
```python
...
# 使用 :visible 选择可见选项
await page.locator('[role="option"]:visible, .el-select-dropdown__item:visible').first.click()
```
...
```python
# ❌ 错误：猜测选项名称，可能导致找不到元素
await page.get_by_text('供应商A', exact=True).click()  # 可能不存在！

# ✅ 正确：使用相对位置选择，避免猜测
await page.locator('[role="option"]').first.click()  # 选择第一个选项

# ✅ 正确：或者使用 Element UI/Ant Design 的通用选择器
