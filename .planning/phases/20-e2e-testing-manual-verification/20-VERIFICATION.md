...
- [x] 打开执行日志/追踪信息
- [x] 验证 `get_data()` 调用已被记录
- [x] 验证 ERP 响应数据已被记录（大响应可截断）
- [x] 验证变量提取过程已被记录（例如 “Extracted imei = XXXXX”）
- [x] 验证变量替换过程在适用场景下已被记录
...
| 15 | **High** | 报告页面缺少前置条件执行信息 | 报告页面不显示前置条件执行状态、耗时、详情展开选项 | [ ] Open (延后到下个里程碑) |
